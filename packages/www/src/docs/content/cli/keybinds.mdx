...
Large patches show a warning instead of rendering automatically. File names and change counts remain available. Press `enter` on the selected file, or click **Load anyway**, to render that patch; loading it may still freeze the terminal. This choice lasts only for the current diff result.

All-files view also limits the combined preview size and defers later patches when that budget is reached. Press `s` to review those files one at a time without loading every patch. Individually oversized patches still require **Load anyway** in single-file view.
...
| `diff.load_patch`       | `return`            | Load deferred diff patch                 |
